...
<div class="jumbotron shadow-std no-rborder slideDown">
    <div class="fadeInjs">
        ...
        <label class="lead" for="capitulo">Seleccione el {{($seccion == 'Capítulo')? 'título':'capítulo'}} al que pertenecen los {{$seccion}}s: </label>
        ...
        @else            
            <div class="lista">
                ...
                @foreach ($secciones as $seccionOut)
                <a href="{{route('editarTitulo', ['id_seccion' => $seccionOut->id])}}"><li class="list-group-item item-lista">{{$seccionOut->numero}}. {{$seccionOut->nombre}}</li></a>
                @endforeach
                </ul>
        </div>
        @endif
        ...
        $("#padre").on('change', function(){
            ...
            $.ajax({
                ...
                for(let i=0; i<respuesta.length; i++){
                    ...
                    $('#listaCA').append(`<a href="${ruta}"><li class="list-group-item item-lista">${respuesta[i].numero}. ${respuesta[i].nombre}</li></a>`);
                }
                if(respuesta.length){
