...
<div class="jumbotron no-rborder shadow-std">
    <div class="header-info">
        ...
        <p class="text-left text-uppercase">creado el <cite>{{$capitulo['contenido']->created_at->format('Y-m-d')}}</cite>, modificado por ultima vez el <cite>{{$capitulo['contenido']->fecha_modificacion}}</cite>.</p>   
    </div>
    <div class="capitulo-header font-weight-bold text-center">
        <h1>{{($capitulo['contenido']->numero)? 'CAPITULO '.$capitulo['contenido']->numero.'.' : '' }}</h1><h1>{{$capitulo['contenido']->nombre}}</h1>
    </div>        
